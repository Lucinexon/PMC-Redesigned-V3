/* ==========================================================================
   ...
   Hybrid persistence + ephemeral multiplayer. Zero-build, retro-safe.

   MODULE MAP
   ──────────────────────────────────────────────────────────────────────────
   QuotaGuard        Firebase Spark armor for the FEW remaining disk writes:
                     cloud saves (players/{uid}) and group-note sync.
                     Presence, movement and chat NEVER touch Firestore.
   AuthManager       Pure username/password ⇄ synthetic email auth, guest
                     (anonymous) mode. Firebase Auth only.
   CloudSync         Writes players/{uid} ONLY on quiz finish, sector clear,
                     or a 60-second background debounce. Never per-keystroke,
                     never per-heartbeat.
   RealtimeBridge    Supabase Realtime channel: presence.track() for lounge
                     avatars + Broadcast for chat. RAM over WebSockets.
                     ZERO database writes. Falls back to local-only roaming
                     if SUPABASE_CONFIG is still the placeholder.
   NotebookManager   Obsidian-grade academic editor: GFM + KaTeX, wikilinks
                     ([[sec01]] / [[Far Lands]]), live TOC, footnotes,
                     Tab indent, auto-list, auto-pairs, metrics, .md export,
                     rich-HTML clipboard (Google Docs / Word). Group rooms
                     still debounce 3s on Firestore group_notes/{code}.
   LoungeEngine      2D pixel-art observation deck. Click-to-walk. Positions
                     stream via RealtimeBridge.track() — no lobby_presence
                     documents, no heartbeat write loop.
   ChatEngine        #global · #essay-group · #study-rooms · #direct-messages
                     via Broadcast. Last 30 packets live in RAM.
   AdminManager      'admin' / 'lucinexon' moderation: golden badges, local
                     purge, global broadcast banner, system audit counts.
   NetworkEngine     Public facade consumed by engine.js (onScreen / onStateSave
                     / forceSave). engine.js showScreen() already calls
                     NetworkEngine.onScreen(id).
...
service cloud.firestore {
  match /databases/{database}/documents {
   ...
   OFFLINE CONTRACT — if FIREBASE_CONFIG / SUPABASE_CONFIG are still
   placeholders, or a CDN is unreachable, this layer degrades:
     • sectors, quizzes, flashcards, minigames, BYOK, scratchpad: intact
     • lounge: local-only roaming (your astronaut still walks)
     • chat: local echo + offline note, no throw
   network.js NEVER throws into engine.js.
   ========================================================================== */
'use strict';
...
/* >>> Paste your Supabase project URL + anon key (Settings → API).
       Until then the lounge falls back to local-only roaming and chat
       shows an offline note — ZERO runtime exceptions. <<< */
const SUPABASE_CONFIG = {
  url: 'YOUR_SUPABASE_URL',
  anonKey: 'YOUR_SUPABASE_ANON_KEY'
};

const SB_CFG_OK = (function () {
  try {
    const u = String(SUPABASE_CONFIG.url || '');
    const k = String(SUPABASE_CONFIG.anonKey || '');
    return u.indexOf('http') === 0 && u.indexOf('YOUR_') < 0 &&
      k.length > 20 && k.indexOf('YOUR_') < 0 && k.indexOf('PASTE') < 0;
  } catch (e) { return false; }
})();
...
function sbOK() { return !!(SB_READY && SB); }
...
function sbInit() {
  if (!SB_CFG_OK) return false;
  const lib = window.supabase;
  if (!lib || typeof lib.createClient !== 'function') return false;
  try {
    SB = lib.createClient(SUPABASE_CONFIG.url, SUPABASE_CONFIG.anonKey, {
      realtime: { params: { eventsPerSecond: 8 } },
      auth: { persistSession: false, autoRefreshToken: false }
    });
    SB_READY = true;
    return true;
  } catch (e) { SB = null; SB_READY = false; return false; }
}

function sessionUid() {
  if (AuthManager.uid) return AuthManager.uid;
  try {
    let id = sessionStorage.getItem('pmc_local_uid');
    if (!id) { id = 'L-' + uid4() + uid4(); sessionStorage.setItem('pmc_local_uid', id); }
    return id;
  } catch (e) { return 'L-' + uid4() + uid4(); }
}

function sessionName() {
  return AuthManager.name || 'GUEST';
}
...
/* ---- CloudSync: 60s background debounce. Presence heartbeats are GONE. ----
   Quiz finish / sector clear call AuthManager.forceSave() (engine.js hooks).
   Movement and chat never funnel through guardedWrite. */
const CloudSync = {
  SAVE_MS: 60000,
  timer: null,
  pending: false,
  tap: function () {
    ...
    this.timer = setTimeout(function () {
      self.timer = null;
      if (self.pending) { self.pending = false; AuthManager.pushProfile(true); }
    }, this.SAVE_MS);
  },
  flush: function () {
    if (this.timer) { clearTimeout(this.timer); this.timer = null; }
    this.pending = false;
    AuthManager.pushProfile(true);
  }
};

/* ========================= [1b] REALTIME BRIDGE =========================
   Supabase Realtime channel. Presence is in-memory on the socket
   (channel.track) and chat is Broadcast. If SUPABASE_CONFIG is unset
   the bridge stays in local mode: the local astronaut still walks,
   chat still echoes your own packets, nothing throws. */
const RealtimeBridge = {
  channel: null,
  status: 'idle',          // idle | live | local
  RING: 30,
  TRACK_MS: 180,
  lastTrack: 0,
  ROOM: 'pmc-deck-v1',
  buf: { global: [], essay: [], study: [], dm: [] },
  wanted: false,

  start: function () {
    this.wanted = true;
    if (this.channel) return;
    if (!sbOK()) { this.status = 'local'; return; }
    const self = this;
    try {
      const ch = SB.channel(this.ROOM, { config: { broadcast: { self: true }, presence: { key: sessionUid() } } });
      ch.on('presence', { event: 'sync' }, function () { self.onPresenceSync(); });
      ch.on('broadcast', { event: 'chat_msg' }, function (pkt) { self.onChat(pkt && pkt.payload); });
      ch.on('broadcast', { event: 'sys_banner' }, function (pkt) { self.onBanner(pkt && pkt.payload); });
  ...
  stop: function () {
    ...
    if (this.channel) {
      ...
      try { if (SB && SB.removeChannel) SB.removeChannel(this.channel); } catch (e) {}
      this.channel = null;
    }
    this.status = 'idle';
  },
  ...
  payloadMe: function () {
    ...
    return {
      ...
      y: LoungeEngine.me.y,
      level: lv,
      admin: AuthManager.admin ? 1 : 0,
      guest: AuthManager.guest ? 1 : 0
    };
  },

  trackMe: function (force) {
    const now = Date.now();
    if (!force && now - this.lastTrack < this.TRACK_MS) return;
    this.lastTrack = now;
    if (!sbOK() || !this.channel || this.status !== 'live') return;
    try { this.channel.track(this.payloadMe()); } catch (e) {}
  },
  ...
  onPresenceSync: function () {
    if (!this.channel || typeof this.channel.presenceState !== 'function') return;
    let state = {};
    try { state = this.channel.presenceState() || {}; } catch (e) { state = {}; }
    LoungeEngine.applyPresence(state);
  },

  chanKey: function (p) {
    if (!p) return 'global';
    if (p.ch === 'essay') return 'essay:' + (p.room || '');
    if (p.ch === 'study') return 'study:' + (p.sub || 'astronomy');
    if (p.ch === 'dm') {
      const pair = [p.u, p.dm].filter(Boolean).sort();
      return 'dm:' + pair.join('_');
    }
    return 'global';
  },
  liveKey: function () {
    if (ChatEngine.channel === 'essay') return 'essay:' + (NotebookManager.room || '');
    if (ChatEngine.channel === 'study') return 'study:' + (ChatEngine.sub || 'astronomy');
    if (ChatEngine.channel === 'dm' && ChatEngine.dm) {
      const pair = [sessionUid(), ChatEngine.dm.uid].sort();
      return 'dm:' + pair.join('_');
    }
    return 'global';
  },
  ...
  pushMsg: function (p) {
    if (!p || !p.t) return;
    const key = this.chanKey(p);
    const ring = this.ring(key);
    ring.push(p);
    if (ring.length > this.RING) ring.splice(0, ring.length - this.RING);
    if (this.liveKey() === key) ChatEngine.render(ring);
  },
  bufForLive: function () { return this.ring(this.liveKey()).slice(); },
  ...
  sendChat: function (payload) {
    ...
    p.n = p.n || sessionName();
    this.pushMsg(p);
    if (!sbOK() || !this.channel || this.status !== 'live') return false;
    try {
      this.channel.send({ type: 'broadcast', event: 'chat_msg', payload: p });
      return true;
    } catch (e) { return false; }
  },
  ...
  onChat: function (p) {
    if (!p || !p.t) return;
    /* self:true already echoed via sendChat → skip exact duplicate from the socket */
    const key = this.chanKey(p);
    const ring = this.ring(key);
    const last = ring[ring.length - 1];
    if (last && last.u === p.u && last.ts === p.ts && last.t === p.t) return;
    this.pushMsg(p);
  },

  onBanner: function (p) {
    if (!p || !p.t) return;
    AdminBanner.show({ t: p.t, by: p.by || 'SYSOP', ts: p.ts || Date.now() });
  },

  sendBanner: function (text, by) {
    const p = { t: text, by: by || 'SYSOP', ts: Date.now() };
    AdminBanner.show(p);
    if (!sbOK() || !this.channel || this.status !== 'live') return false;
    try { this.channel.send({ type: 'broadcast', event: 'sys_banner', payload: p }); return true; }
    catch (e) { return false; }
  }
};
...
const AuthManager = {
  ...
  authResolved: false, lastPush: 0, pushPending: false, PUSH_MIN_MS: 60000,
  ...
  ensureAuth: function (why) {
    /* Wait until Firebase has finished restoring any existing session. */
    ...
    AUTH.signInAnonymously()
      .then(function () {
        self.busy = false;
        say(
          '<b>GUEST LINK ESTABLISHED</b><br>' +
          (why || 'Live deck access granted — register anytime from ACCOUNT.'),
          'power',
          3600
        );
      })
  ...
  onStateSave: function () {                       // engine.js save() hook — 60s debounce only
    if (this.user && !this.guest) CloudSync.tap();
  },
  forceSave: function (reason) {
    if (this.user && !this.guest) CloudSync.flush();
  },
  updateUI: function () {
    ...
    if (pw && lw) {
      ...
      if (signed) {
        ...
        q('#apSyncTx').textContent = 'CLOUD PROFILE: SYNCED · WRITES TODAY: ' + QuotaGuard.ledger.w + '/' + QuotaGuard.BUDGET + ' (saves on quiz / sector / 60s)';
      }
    }
  },
  openModal: function () {
...
const NotebookManager = {
  ...
  init: function () {
    ...
    ed.addEventListener('keydown', function (e) { self.onKeydown(e); });
    const tocBtn = q('#nbTocToggle');
    if (tocBtn) tocBtn.addEventListener('click', function () {
      const wrap = q('#nbTocWrap');
      if (!wrap) return;
  ...
  tool: function (what) {
    ...
    if (what === 'h1' || what === 'h2' || what === 'h3' || what === 'quote') {
      ...
      const body = sel || 'sec01';
      const out = v.slice(0, s) + '[[' + body + ']]' + v.slice(e);
      set(out, s + 2, s + 2 + body.length);
    } else if (what === 'ul') {
      const lineStart = v.lastIndexOf('\n', s - 1) + 1;
      const already = v.slice(lineStart, lineStart + 2) === '- ';
      const next = already
        ? v.slice(0, lineStart) + v.slice(lineStart + 2)
        : v.slice(0, lineStart) + '- ' + v.slice(lineStart);
  ...
  updateMetrics: function () {
    ...
    const pages = words / 275;   // standard double-spaced academic page
    q('#nbWords').textContent = words + ' WORDS';
    q('#nbChars').textContent = chars + ' CHARS';
    const read = words ? Math.max(1, Math.round(mins)) : 0;
    q('#nbRead').textContent = '~' + read + ' MIN READ';
  ...
  slugify: function (s) {
    return String(s || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 48) || 'h';
  },
  renderToc: function (text) {
    const nav = q('#nbToc');
    if (!nav) return;
    const items = [];
    String(text || '').split('\n').forEach(function (line) {
      const m = line.match(/^(#{1,3})\s+(.+?)\s*$/);
      if (m) items.push({ lv: m[1].length, t: m[2].replace(/[#*_`\[\]]/g, '').trim() });
    });
    if (!items.length) {
      nav.innerHTML = '<p class="nb-toc-empty">Headings appear here as you write <code>#</code> <code>##</code> <code>###</code>.</p>';
      return;
    }
    const seen = {};
    nav.innerHTML = items.map(function (it) {
      let slug = NotebookManager.slugify(it.t);
      seen[slug] = (seen[slug] || 0) + 1;
      if (seen[slug] > 1) slug += '-' + seen[slug];
      return '<button type="button" class="nb-toc-item l' + it.lv + '" data-jump="' + esc(slug) + '">' + esc(it.t) + '</button>';
    }).join('');
    qa('.nb-toc-item', nav).forEach(function (b) {
      b.addEventListener('click', function () {
        const panes = q('.nb-panes');
        if (panes && window.matchMedia && window.matchMedia('(max-width:860px)').matches) {
          NotebookManager.tab('preview');
        }
        const target = q('#nb-h-' + b.dataset.jump, q('#nbPreview'));
        if (target) target.scrollIntoView({ behavior: (typeof matchMedia !== 'undefined' && matchMedia('(prefers-reduced-motion: reduce)').matches) ? 'auto' : 'smooth', block: 'start' });
      });
    });
  },
  /* ---- editor ergonomics: Tab indent, list continue, auto-pairs ---- */
  onKeydown: function (e) {
    const ed = this.editor();
    if (!ed) return;
    if (e.key === 'Tab') { e.preventDefault(); this.indent(!e.shiftKey); return; }
    if (e.key === 'Enter' && !e.shiftKey && this.continueList(e)) return;
    this.autoPair(e);
  },
  indent: function (into) {
    const ed = this.editor();
    if (!ed) return;
    const v = ed.value, s = ed.selectionStart, e = ed.selectionEnd;
    const start = v.lastIndexOf('\n', s - 1) + 1;
    const end = (e > s && v.charAt(e - 1) === '\n') ? e - 1 : v.indexOf('\n', e);
    const blockEnd = end < 0 ? v.length : end;
    const block = v.slice(start, blockEnd);
    const lines = block.split('\n');
    let deltaStart = 0, deltaEnd = 0;
    const next = lines.map(function (ln, i) {
      if (into) {
        if (i === 0) deltaStart += 2;
        deltaEnd += 2;
        return '  ' + ln;
    ...
    ed.value = v.slice(0, start) + next + v.slice(blockEnd);
    ed.selectionStart = Math.max(start, s + deltaStart);
    ed.selectionEnd = Math.max(ed.selectionStart, e + deltaEnd);
    this.onInput();
  },
  continueList: function (e) {
    ...
    if (!rest.trim()) {
      ed.value = v.slice(0, start) + v.slice(s);
    ...
    if (num) next = (parseInt(num[1], 10) + 1) + '. ';
    const ins = '\n' + indent + next;
    ed.value = v.slice(0, s) + ins + v.slice(ed.selectionEnd);
  ...
  autoPair: function (e) {
    ...
    const ed = this.editor();
    if (!ed) return;
    const v = ed.value, s = ed.selectionStart, t = ed.selectionEnd, sel = v.slice(s, t);
    if (e.key === '*' && !e.metaKey && !e.ctrlKey) {
      e.preventDefault();
      const wrap = '*' + (sel || '') + '*';
      ed.value = v.slice(0, s) + wrap + v.slice(t);
      ed.selectionStart = s + 1;
      ed.selectionEnd = s + 1 + sel.length;
      this.onInput();
      return;
    }
    if (e.key === '$' && !e.metaKey && !e.ctrlKey) {
      if (v.charAt(s - 1) === '$') return;
      e.preventDefault();
      const wrap = (sel.indexOf('\n') >= 0 ? '$$\n' + sel + '\n$$' : '$' + sel + '$');
      ed.value = v.slice(0, s) + wrap + v.slice(t);
      ed.selectionStart = s + (sel.indexOf('\n') >= 0 ? 3 : 1);
      ed.selectionEnd = ed.selectionStart + sel.length;
      this.onInput();
      return;
    }
    if (!pairs[e.key] || e.metaKey || e.ctrlKey) return;
    if (e.key === '[' && v.charAt(s - 1) === '[') return;
    e.preventDefault();
    const close = pairs[e.key];
    ed.value = v.slice(0, s) + e.key + sel + close + v.slice(t);
    ed.selectionStart = s + 1;
    ed.selectionEnd = s + 1 + sel.length;
    this.onInput();
  },

  /* ---- academic markdown: GFM + footnotes + wikilinks + heading ids ---- */
  nbToHTML: function (md) {
    let src = String(md || '');
    const notes = [];
    const noteMap = {};
    src = src.replace(/^\[\^([^\]]+)\]:\s*(.+)$/gm, function (m, id, body) {
      noteMap[id] = body;
      return '';
    });
    const wikiHold = [];
    src = src.replace(/\[\[([^\]]+)\]\]/g, function (m, t) {
      wikiHold.push(t);
      return '\u0002W' + (wikiHold.length - 1) + '\u0002';
    });
    src = src.replace(/\[\^([^\]]+)\]/g, function (m, id) {
      notes.push(id);
      return '\u0002F' + (notes.length - 1) + '\u0002';
    });
    let html;
    if (typeof window.mdBlock === 'function') html = window.mdBlock(src);
    else if (typeof window.marked !== 'undefined') {
      try { html = window.marked.parse(src, { gfm: true, breaks: true }); }
      catch (e) { html = '<p>' + esc(src).replace(/\n/g, '<br>') + '</p>'; }
    } else html = '<p>' + esc(src).replace(/\n/g, '<br>') + '</p>';
    html = html.replace(/\u0002W(\d+)\u0002/g, function (m, i) {
      const t = wikiHold[+i] || '';
      return '<button type="button" class="wiki-badge" data-wiki="' + esc(t) + '">[[' + esc(t) + ']]</button>';
    });
    html = html.replace(/\u0002F(\d+)\u0002/g, function (m, i) {
      const id = notes[+i] || '';
      return '<sup class="nb-fn"><a href="#fn-' + esc(NotebookManager.slugify(id)) + '">' + esc(id) + '</a></sup>';
    });
    const seenH = {};
    html = html.replace(/<(h[1-3])>([\s\S]*?)<\/h[1-3]>/gi, function (m, tag, inner) {
      const plain = inner.replace(/<[^>]+>/g, '').trim();
      let slug = NotebookManager.slugify(plain);
      seenH[slug] = (seenH[slug] || 0) + 1;
      if (seenH[slug] > 1) slug += '-' + seenH[slug];
      return '<' + tag + ' id="nb-h-' + slug + '">' + inner + '</' + tag + '>';
    });
    const ids = Object.keys(noteMap);
    if (ids.length) {
      html += '<div class="nb-biblio"><h3>Notes & Bibliography</h3><ol>';
      ids.forEach(function (id) {
        html += '<li id="fn-' + esc(NotebookManager.slugify(id)) + '"><b>[' + esc(id) + ']</b> ' + esc(noteMap[id]) + '</li>';
      });
      html += '</ol></div>';
    }
    return html;
  ...
  renderPreview: function (force) {
    ...
    const md = ed.value || '';
    if (!md.trim()) {
      pv.innerHTML = '<p class="small">The formatted preview renders here — headings, tables, KaTeX, wikilinks, footnotes and citations included.</p>';
      this.renderToc('');
      return;
    }
    pv.innerHTML = this.nbToHTML(md);
    if (typeof window.typeset === 'function') window.typeset(pv);
    this.bindPreview(pv);
  },
  bindPreview: function (root) {
    const self = this;
    qa('.wiki-badge', root).forEach(function (b) {
      b.addEventListener('click', function (ev) {
        ev.preventDefault();
        self.goWiki(b.getAttribute('data-wiki') || '');
      });
    });
  },
  resolveWiki: function (raw) {
    const t = String(raw || '').trim();
    if (!t) return null;
    const low = t.toLowerCase();
    const secNum = low.match(/^sec(?:tor)?\s*0*(\d+)$/i) || low.match(/^s0*(\d+)$/i);
    if (secNum) {
      const id = 'sec' + parseInt(secNum[1], 10);
      if (document.getElementById(id)) return { screen: id };
    }
    if (document.getElementById(low)) return { screen: low };
    const nav = (typeof window.NAVMAP !== 'undefined') ? window.NAVMAP : null;
    if (nav) {
      const keys = Object.keys(nav);
      for (let i = 0; i < keys.length; i++) {
        const n = nav[keys[i]];
        if (n && n.label && n.label.toLowerCase() === low) return { screen: keys[i] };
        if (n && n.sub && String(n.sub).toLowerCase().indexOf(low) >= 0) return { screen: keys[i] };
      }
    }
    const sectors = (typeof window.SECTORS !== 'undefined') ? window.SECTORS : [];
    for (let i = 0; i < sectors.length; i++) {
      const s = sectors[i];
      if (s.label && s.label.toLowerCase() === low) return { screen: s.id };
      if (s.title && s.title.toLowerCase().indexOf(low) >= 0) return { screen: s.id };
      const blocks = s.blocks || [];
      for (let j = 0; j < blocks.length; j++) {
        const b = blocks[j];
        if (b.id && b.id.toLowerCase() === low) return { screen: s.id, block: b.id };
        if (b.title && b.title.toLowerCase().indexOf(low) >= 0) return { screen: s.id, block: b.id };
        if (b.tags && b.tags.some(function (tg) { return String(tg).toLowerCase() === low; })) {
          return { screen: s.id, block: b.id };
        }
      }
    }
    return null;
  },
  goWiki: function (target) {
    const hit = this.resolveWiki(target);
    if (!hit || !hit.screen) {
      say('<b>WIKILINK MISS</b><br>No sector or block matched [[' + esc(target) + ']]. Try [[sec01]] or a block title.', 'search', 3400);
      blip('bad');
      return;
    }
    if (typeof window.showScreen === 'function') window.showScreen(hit.screen);
    if (hit.block) {
      setTimeout(function () {
        const el = document.getElementById(hit.block);
  ...
  richHTML: function () {
    ...
    const md = ed ? ed.value : '';
    let html;
    if (typeof this.nbToHTML === 'function') html = this.nbToHTML(md);
    else if (typeof window.mdBlock === 'function') html = window.mdBlock(md);
...
/* ========================= [4] LOUNGE ENGINE =========================
   ...
   walk; positions stream over Supabase Presence (channel.track) — ZERO
   Firestore writes. Unconfigured SUPABASE_CONFIG → local-only roaming. */
const LoungeEngine = {
  ...
  enter: function () {
    ...
    this.beat(true);
    if (!this.beatT) this.beatT = setInterval(function () { self.beat(false); }, 400);
  ...
  onClick: function (e) {
    ...
    this.me.walk = true;
    blip('click');
    RealtimeBridge.trackMe(true);
  },

  beat: function (force) {
    /* RAM-only presence tick — WebSocket track(), never a document write. */
    if (this.me.walk || force) RealtimeBridge.trackMe(!!force);
  },

  applyPresence: function (state) {
    const next = {};
    const self = this;
    const me = sessionUid();
    Object.keys(state || {}).forEach(function (key) {
      ...
      next[uid] = {
        n: String(d.name || d.n || 'OP').slice(0, 14),
        lv: d.level || d.lv || 1,
        a: !!(d.admin || d.a),
        x: d.x || 240,
        y: d.y || 244,
  ...
  onlineOperators: function () {
    const out = [{ uid: sessionUid(), name: sessionName(), me: true, admin: AuthManager.admin, lv: (typeof window.levelOf === 'function' && engS()) ? window.levelOf(engS().xp || 0) + 1 : 1 }];
    const self = this;
    Object.keys(this.players).forEach(function (uid) {
      out.push({ uid: uid, name: self.players[uid].n, me: false, admin: self.players[uid].a, lv: self.players[uid].lv });
    });
    return out;
  },
  ...
  updateWhoami: function () {
    ...
    if (!el) return;
    const live = sbOK() && RealtimeBridge.status === 'live';
    if (!sbOK()) el.innerHTML = 'IDENT: <b>' + esc(sessionName()) + '</b> · <span style="color:var(--gold)">LOCAL DECK</span>';
    else if (!live) el.innerHTML = 'IDENT: <b>' + esc(sessionName()) + '</b> · <span style="color:var(--gold)">SOCKET IDLE</span>';
    else el.innerHTML = 'IDENT: <b style="color:' + (AuthManager.admin ? 'var(--gold)' : 'var(--cyan)') + '">' +
      esc(sessionName()) + (AuthManager.admin ? ' ★SYSOP' : (AuthManager.guest ? ' ·GUEST' : '')) + '</b> · LIVE SOCKET';
  },
  ...
  renderRoster: function () {
    ...
    const chip = q('#lobbyCountChip');
    if (!sbOK()) {
      if (chip) chip.textContent = 'LOCAL';
      ro.innerHTML = '<span class="small">LOCAL DECK — roam freely. Paste SUPABASE_CONFIG in js/network.js to see other operators over WebSockets (zero database writes).</span>';
      return;
    }
    if (!live) {
      if (chip) chip.textContent = 'CONNECTING';
      ro.innerHTML = '<span class="small">Opening realtime socket…</span>';
      return;
    }
    if (chip) chip.textContent = ops.length + ' ONLINE';
    ro.innerHTML = ops.map(function (o) {
      return '<button class="lro' + (o.me ? ' me' : '') + (o.admin ? ' admin' : '') + '" data-dmuid="' + esc(o.uid) +
        '" data-dmname="' + esc(o.name) + '" title="Message ' + esc(o.name) + '"><span class="tdot' + (o.me ? '' : '') + '"></span>' +
        esc(o.name) + (o.admin ? ' ★' : '') + '<span class="lv">LV' + (o.lv || 1) + '</span><span class="tx-tag live">WS</span></button>';
    }).join('');
    qa('.lro', ro).forEach(function (b) {
      b.addEventListener('click', function () {
        if (b.dataset.dmuid === sessionUid()) return;
  ...
  draw: function (t) {
    ...
    /* status text when the socket is local-only */
    if (!sbOK() || RealtimeBridge.status !== 'live') {
      c.fillStyle = 'rgba(4,8,25,.72)'; c.fillRect(0, 78, this.W, 34);
      c.fillStyle = !sbOK() ? '#ffd166' : '#3fe0ff';
      c.font = 'bold 9px Silkscreen, "Press Start 2P", monospace';
      c.textAlign = 'center';
      c.fillText(!sbOK() ? 'LOCAL DECK — PASTE SUPABASE_CONFIG FOR LIVE OPERATORS'
                        : 'OPENING REALTIME SOCKET…', this.W / 2, 98);
      c.textAlign = 'left';
    }
...
/* ========================= [5] CHAT ENGINE =========================
   Multi-channel realtime chat over Supabase Broadcast. Packets live in a
   30-message RAM ring — they never become Firestore documents. 1s send
   rate limiter. Admin × hides a packet locally (nothing on disk to delete). */
const ChatEngine = {
  channel: 'global', sub: null, dm: null,
  lastSend: 0, RATE_MS: 1000,
  TOPICS: ['astronomy', 'physics', 'gaming-lore', 'mathematics', 'history', 'psychology'],

  colOf: function () { return null; },  // retired Firestore collections
  chanLabel: function () {
  ...
  openDM: function (uid, name) {
    if (!uid || uid === sessionUid()) { this.open('dm', null); return; }
  ...
  renderSubchips: function () {
    ...
    if (this.channel === 'study') {
      ...
      box.hidden = false;
      const ops = sbOK() ? LoungeEngine.onlineOperators() : [];
  ...
  attach: function (quiet) {
    ...
    if (this.channel === 'dm' && !this.dm) {
      ...
      log.innerHTML = '<div class="chat-sys">DIRECT MESSAGE RELAY — pick an operator from the deck roster or the chips above.' +
        (ops.length ? '' : '<br>No other operators currently on deck — they appear over the live socket.') + '</div>';
      return;
    }
    this.render(RealtimeBridge.bufForLive());
  },
  ...
  render: function (docs) {
    ...
    docs = docs || [];
    if (!docs.length) {
      let sys = esc(this.chanLabel()) + ' is quiet. Break the silence, operator.';
      if (!sbOK()) sys = 'OFFLINE COMMS — packets stay on this device. Paste SUPABASE_CONFIG to broadcast live (zero database writes).';
      log.innerHTML = '<div class="chat-sys">' + sys + '</div>';
      return;
    }
    const self = this;
    const admin = AdminManager.isAdmin();
    log.innerHTML = docs.map(function (m, i) {
      const mine = m.u === sessionUid();
      return '<div class="cmsg' + (mine ? ' mine' : '') + '">' +
        ...
        (admin ? '<button class="cdel show" data-del="' + i + '" title="Hide packet" aria-label="Hide message">×</button>' : '') +
        '</div>' +
  ...
  send: function () {
    ...
    if (now - this.lastSend < this.RATE_MS) {
      say('<b>TRANSMISSION THROTTLED</b><br>1-second cooldown between packets.', 'speech', 2200);
      blip('bad');
      return;
    }
    this.lastSend = now;
    RealtimeBridge.sendChat({
      ch: this.channel,
      sub: this.sub,
      room: NotebookManager.room || '',
      dm: this.dm ? this.dm.uid : '',
      n: sessionName(),
      u: sessionUid(),
      t: text,
      ts: now,
      a: AuthManager.admin ? 1 : 0,
      g: AuthManager.guest ? 1 : 0
    });
    input.value = '';
    blip('flip');
    if (!sbOK()) say('<b>LOCAL ECHO</b><br>Packet stayed on this device — live broadcast needs SUPABASE_CONFIG.', 'speech', 2800);
  },

  del: function (idx) {
    if (!AdminManager.isAdmin()) { say('Unauthorized.', 'shield', 2000); return; }
    const ring = RealtimeBridge.ring(RealtimeBridge.liveKey());
    if (idx >= 0 && idx < ring.length) ring.splice(idx, 1);
    this.render(ring);
    blip('bad');
  },

  sweep: function () { /* no Firestore docs to expire */ },

  renderNote: function () {
    const el = q('#chatNote');
    if (!el) return;
    let note = '1s rate limit · ephemeral WebSocket broadcast · last 30 packets live in RAM';
    if (!sbOK()) note = 'LOCAL COMMS — paste SUPABASE_CONFIG in js/network.js to open the live socket. Zero database writes either way.';
    else if (RealtimeBridge.status !== 'live') note = 'Opening realtime socket…';
...
const AdminManager = {
  ...
  broadcast: function () {
    ...
    const sent = RealtimeBridge.sendBanner(text, AuthManager.name || 'SYSOP');
    if (inp) inp.value = '';
    if (fbOK()) {
    ...
    say('<b>BROADCAST DISPATCHED</b><br>' + (sent ? 'Banner deployed over the live socket.' : 'Banner shown locally (socket idle).'), 'antenna', 3400);
    blip('level');
  },
  auditTick: function () {
  ...
  if (ab) ab.addEventListener('click', function () {
    if (!fbOK()) {
      ...
      if (res) {
        ...
        res.textContent = 'OFFLINE MODE — paste FIREBASE_CONFIG for accounts & cloud saves. Lounge presence / chat need SUPABASE_CONFIG (WebSockets, zero writes).';
      }
      return;
    }
    AuthManager.openModal();
...
function boot() {
  try {
    ...
    if (live) RealtimeBridge.start();
    window.addEventListener('beforeunload', function () {
      try {
        NotebookManager.saveScratch();
        NoteSync.stop();
        LoungeEngine.clearPresence();
      } catch (e) {}
    });
  } catch (e) {
...
window.NetworkEngine = {
  ...
  forceSave: function (reason) { AuthManager.forceSave(reason); },
  openAuth: function () { AuthManager.openModal(); },
  online: function () { return fbOK(); },
  socket: function () { return sbOK() && RealtimeBridge.status === 'live'; },
